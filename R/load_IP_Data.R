...
load_IP_Data <- function(countFile, sampleTable)
{
  ...
  Count_file <- read.table(countFile, sep= "\t", header = T, stringsAsFactors=TRUE, quote="", fill=FALSE)
  Count_file_1 <- Count_file[,2:ncol(Count_file)]
  row.names(Count_file_1) <- Count_file[,1]
  Sample_table <- read.table(sampleTable, sep= "\t", header = T, stringsAsFactors=TRUE, quote="", fill=FALSE)
